...
class CapturedImageViewModel: ObservableObject {
    ...
    @Published var recipes: [RecipeModel] = []
    @Published var recipeGenerationComplete = false
    
    private let recipeViewModel: RecipeViewModel
    private var cancellables = Set<AnyCancellable>()
    private var initialRecipeCount = 0
    
    init(recipeViewModel: RecipeViewModel) {
        self.recipeViewModel = recipeViewModel
        self.initialRecipeCount = recipeViewModel.recipes.count
        
        // Monitor recipe changes to detect when new recipes are added
        recipeViewModel.$recipes
            .sink { [weak self] recipes in
                guard let self = self else { return }
                
                if !self.isLoading && recipes.count > self.initialRecipeCount {
                    self.recipes = recipes
                    self.recipeGenerationComplete = true
                }
            }
            .store(in: &cancellables)
        ...
        // Monitor when generation is completed
        recipeViewModel.$generationCompleted
    ...
    func processImage(image: UIImage) {
        ...
        recipeGenerationComplete = false
